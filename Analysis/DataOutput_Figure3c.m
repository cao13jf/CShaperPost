  % Mission 1 : Data Output of Figure 3c
    ...
    load('E:\Project 12 - Wnt & Membrane\CJF\Paper Revision - PLUSPLUS\Revision20200424 - QueryUpdate\WorkSpace_Query') ; SourceData={}                 ;
    load('E:\Project 1 - C.elegans Resource\11-Patch\Problem 1 - WT Cycle\WorkSpace_CellName') ; SSS=Sequence0          ; SourceData{1,1}='Cell Name'   ;
    SourceData{1,2}='Average Cell/Nucleus Position ([x,y,z] / ¦Ìm)' ; SourceData{1,3}='Spatial Deviation (¦¤r / ¦Ìm)'    ; SourceDataPLUS={}             ;
    SourceDataPLUS{1,1}='Effective Contact'    ; SourceDataPLUS{2,1}='Cell i'  ; SourceDataPLUS{2,2}='Cell j'            ;
for k1=1:size(C,1)
for k2=1:size(C,2)
...
for n=8
    Parameter=[] ; CELLNAME={}    ; NUM=1      ; num=2     ;
...
if  Criteria{1,n}{k0,1}{k1,k2}>1-1e-5
    CELLNAME{end+1,1}=CellName{k0,1}{k1,k2}    ; NUM=NUM+1 ; SourceData{NUM,1}=CellName{k0,1}{k1,k2}       ;
end
end
end
end
...
for j1=1:length(Sequence0{8,1})-1
for j2=j1+1:length(Sequence0{8,1})
if  j1~=j2
    for k0=1:length(CellName)
    for k1=1:size(CellName{k0,1},1)
    for k2=1:size(CellName{k0,1},2)
    if  strcmp(CellName{k0,1}{k1,k2},Sequence0{8,1}{j1,1})==1
        g1=Criteria{4,n}{k0,1}{k1,k2} ; cell1=CellName{k0,1}{k1,k2} ;
    end
    if  strcmp(CellName{k0,1}{k1,k2},Sequence0{8,1}{j2,1})==1
        g2=Criteria{4,n}{k0,1}{k1,k2} ; cell2=CellName{k0,1}{k1,k2} ;
    end
    end
    end
    end
    if  Sequence0{10,1}(j1,j2)==1 || isnan(Sequence0{10,1}(j1,j2))==1
        plot3([g1(2),g2(2)],[g1(3),g2(3)],[g1(1),g2(1)],'-','linewidth',4,'color',[0.5,0.5,0.5]) ; hold on ;
        num=num+1 ; SourceDataPLUS{num,1}=cell1 ; SourceDataPLUS{num,2}=cell2                    ;
    end
end
end
end
...
if  isnumeric(CellName{k0,1}{k1,k2})==0 && strcmp(CellName{k0,1}{k1,k2},CELLNAME{j,1})==1
    g=Criteria{4,n}{k0,1}{k1,k2}   ; X=sprintf('%2.2f',Criteria{4,n}{k0,1}{k1,k2}(2))        ; Y=sprintf('%2.2f',Criteria{4,n}{k0,1}{k1,k2}(3)) ; Z=sprintf('%2.2f',Criteria{4,n}{k0,1}{k1,k2}(1))      ;
    RRR=sprintf('%2.4f',Criteria{3,n}{k0,1}{k1,k2}) ;
    ...
    view(-40,20) ; axis equal      ; grid off ; SourceData{1+j,2}=['[',X,',',Y,',',Z,']']    ; SourceData{1+j,3}=RRR ;
end
end
end
end
    ...
    print(gcf,'-r600','-dpng',['Connection.png'])                ;
end
    xlswrite('Figure 3c.xls',SourceData    ,'Position & Deviation')               ;
    xlswrite('Figure 3c.xls',SourceDataPLUS,'Effective Contact')                  ;