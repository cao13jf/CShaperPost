  % Mission 1 : Data Output of Figure 2a
    ...
    SourceData=cell(size(Data,1)+1,size(Data,3)*4) ; R0=1 ; C0=-3   ;
    TP={'Time Point 24';'Time Point 34';'Time Point 44';'Time Point 54';'Time Point 64';'Time Point 74';'Time Point 84'}      ;
for I=1:size(Data,1)
...
for j=1:size(Data,3)
    X=X+2 ; XXX=X    ; C0=C0+4    ; SourceData{R0,C0}=TP{j,1}       ;
    SourceData{R0+1,C0}='Method'  ; SourceData{R0+1,C0+1}='Dice Ratio of Samples 02-04 (%)' ;
    SourceData{R0+1,C0+2}='p-value (Comparison to CShaper)'         ;
for I=1:7 % 1:size(Data,1)
    X=X+1 ; value=[] ; VALUE=[]   ; SourceData{R0+1+I,C0}=File(I).name ; NUM=''             ;
for i=[1,3,2] % 1:size(Data,2)
    value=[value,Data(I,i,j)]     ; NUM=[NUM,sprintf('%2.2f',Data(I,i,j)),',']              ;
end
    SourceData{R0+1+I,C0+1}=NUM(1,1:end-1) ;
if  I~=length(File)
...
for i=1:size(Data,2)
    ...
    [p,h,stats]=ranksum(value,VALUE,'alpha',0.01,'method','exact','tail','left') ;
if  h==1
    plot([0.8,0,-0.4]+(X-1+1.5+XXX+6+1.5)/2,(158.5-8.25*I)*ones(1,3),'k*','linewidth',0.8,'markersize',6) ; hold on                ; SourceData{R0+1+I,C0+2}=sprintf('%2.2f',p) ;
else if h==0
    [p,h,stats]=ranksum(value,VALUE,'alpha',0.05,'method','exact','tail','left') ;
if  h==1
    plot([0,0.6]-0.3+(X-1+1.5+XXX+6+1.5)/2,(158.5-8.25*I)*ones(1,2) ,'k*','linewidth',0.8,'markersize',6) ; hold on                ; SourceData{R0+1+I,C0+2}=sprintf('%2.2f',p) ;
else if h==0
    [p,h,stats]=ranksum(value,VALUE,'alpha',0.10,'method','exact','tail','left') ;
if  h==1
    plot([0.8]-0.8+(X-1+1.5+XXX+6+1.5)/2,(158.5-8.25*I)*ones(1,1)   ,'k*','linewidth',0.8,'markersize',6) ; hold on                ; SourceData{R0+1+I,C0+2}=sprintf('%2.2f',p) ;
else if h==0
    text([0.8]-1.4+(X-1+1.5+XXX+6+1.5)/2,(158.5-8.25*I)*ones(1,1)+0.6,'n.s.','Fontname','Arial','Fontsize',11,'FontWeight','Bold') ; SourceData{R0+1+I,C0+2}=sprintf('%2.2f',p) ;
    end
end
end
end
...
for t=1:length(timepoint)
    ...
    xlswrite('Figure 2a.xls',SourceData,'Sheet1')       ;