  % Mission 1 : Data Output of Supplementary Table 4
...
for I=1:size(EMBRYO,1)
    ...
    VAR=std(VOLUME)./mean(VOLUME) ; VOL=mean(VOLUME)             ; SourceData1{2,2+size(EMBRYO,1)}=[]     ;
for iii=1:length(VAR)
    SourceData3{2+iii,2}=sprintf('%2.4f',VAR(1,iii))             ;
    SourceData1{2+iii,2+size(EMBRYO,1)}=VOL(1,iii) ;
end
    SourceData1=[SourceData1(:,1),SourceData1(:,end),SourceData1(:,2:end-1)] ; SourceData1{2,2}='Average' ;
    xlswrite('Supplementary Table 4.xls',SourceData1,'Table S4') ;