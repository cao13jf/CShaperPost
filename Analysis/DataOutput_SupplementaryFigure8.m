  % Mission 1 : Data Output of Supplementary Figure 8
...
if  size(CellName{k0,1},2)==1
    ...
    if  isnumeric(CellName{k0,1}{k1,k2})==0
        ...
        if  strcmp(CellName{k0,1}{k1,k2},Ratio{i,1})==1
            lineage=[lineage;str2num(Ratio{i,2})] ; name{end+1,1}=CellName{k0,1}{k1,k2}        ;
        end
        end
    end
    end
...
if  size(CellName{k0,1},2)~=1
    ...
    if  isnumeric(CellName{k0,1}{k1,k2})==0
        ...
        Lineage{end+1,1}=lineage ; Lineage{end,2}=name ;
end
        save('WorkSpace_Lineage','Lineage','-v7.3')    ;
        
    load('WorkSpace_Lineage') ; LINE=1.25 ; WIDTH=0.2  ; figure  ; set(gcf,'position',[50,50,500,500])    ; SourceData={}       ;
    SourceData{1,3*1-2}='AB Lineage' ; SourceData{1,3*2-2}='MS Lineage' ; SourceData{1,3*3-2}='E Lineage' ; SourceData{1,3*4-2}='C Lineage' ;
for k0=1:size(Lineage,1)-3
    data=sort(Lineage{k0,1})' ; SourceData{2,3*k0-2}='Cell Name' ; SourceData{2,3*k0-1}='Cell Shape Irregularity ¦Ç (Averaged)' ; n=2       ;
for k1=1:length(Lineage{k0,2})
    n=n+1 ; SourceData{n,3*k0-2}=Lineage{k0,2}{k1,1}   ; SourceData{n,3*k0-1}=sprintf('%2.4f',Lineage{k0,1}(k1,1))     ;
end
...
if  k0~=1
    ...
    xlswrite('Supplementary Figure 8.xls',SourceData,'Figure S8')             ;