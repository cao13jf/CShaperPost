  % Mission 1 : Data Output of Figure 2b
    ...
    SampleName={'Sample 02';'Sample 03';'Sample 04'} ; SourceData=cell(9,length(SampleName)*9) ; R0=1  ; C0=-3         ;
for i=[1,3,2]
    X=X+5     ; C0=C0+4 ; SourceData{R0,C0}=SampleName{find([1,3,2]==i),1} ;
    SourceData{R0+1,C0}='Method'  ; SourceData{R0+1,C0+1}='Hausdorff Distance of Samples 02-04 (\mum)' ;
...
for I=1:length(File)
    SourceData{R0+1+I,C0}=File(I).name                 ;
    load(['D:\STORAGE\Project 12 - Wnt & Membrane\CJF\Paper Revision - PLUSPLUS\Revision20200530 - CAO FigureUpdate_DATA\',File(I).name,'\',Embryo(i).name])     ;
    ...
    plot([X-0.5,X+0.5],(mean(R)-std(R)*[1])*[1,1],'-','linewidth',2,'color',c(I,:))   ; hold on ; NUM=''   ;
for iii=1:length(Distance)
    NUM=[NUM,sprintf('%2.2f',Distance(iii,1)),',']     ;
end 
    SourceData{R0+1+I,C0+1}=NUM ; SourceData{R0+1+length(File),C0+2}='/'       ;
if  I~=length(File)
...
if  h==1
    plot([0,1.6,3.2]-1.6+(XXX+27+X)/2,([57.5]-I*6+47.5)*ones(1,3),'k*','linewidth',1,'markersize',6.5) ; hold on ; SourceData{R0+1+I,C0+2}=num2str(p) ;
else if h==0
    [p,h,stats]=ranksum(R,RRR,'alpha',0.05,'tail','right') ;
if  h==1
    plot([0,1.6]-0.8+(XXX+27+X)/2,([57.5]-I*6+47.5)*ones(1,2)    ,'k*','linewidth',1,'markersize',6.5) ; hold on ; SourceData{R0+1+I,C0+2}=num2str(p) ;
else if h==0
    [p,h,stats]=ranksum(R,RRR,'alpha',0.10,'tail','right') ;
if  h==1
    plot(0+(XXX+27+X)/2,([57.5]-I*6+47.5)*ones(1,1)              ,'k*','linewidth',1,'markersize',6.5) ; hold on ; SourceData{R0+1+I,C0+2}=num2str(p) ;
end
end
end
end
    ...
    xlswrite('Figure 2b.xls',SourceData,'Sheet1')                           ;